...
if(!interaction.isButton() && !interaction.isSelectMenu()) return;
      ...
      if(interaction.customId == `create_ticket${s}`) {
        ...
        if(!interaction.guild.me.permissions.has("MANAGE_CHANNELS")) return interaction.reply({ content: `${client.emoji.wrong} **Non sono riuscito a creare il ticket!**\n> *Assicurati di avere l'autorizzazione \`MANAGE_CHANNELS\`!*`, ephemeral: true })
        ...
        .setTitle(`${client.emoji.wrong} Hai già un Ticket Aperto!`)
        .setDescription(`***Hai già un ticket aperto in ${checkTickets}! Si prega di chiuderlo prima!***`)
        .setFooter(`Ticketing by ! Simo#0272`, interaction.guild.iconURL())
        ...
        .addComponents([
         ...
         await interaction.reply({ content: `<a:loading_:906786750494564353> **Creazione del ticket (questo potrebbe richiedere alcuni secondi)...**`, ephemeral: true })
        interaction.guild.channels.create(`${tcname || `ticket-${interaction.user.username}`}`, {
          ...
          interaction.editReply({ content: `${client.emoji.wrong} **An Error Occured While Creating Your Ticket!**\n> *ErrCode: \`hHa_8\`*`, components: [reasons], ephemeral: true })
                ...
                await wait(1000)
                await interaction.editReply({ content: `${client.emoji.correct} **Hai creato con successo il tuo ticket in ${channel}**`, ephemeral: true })
        ...
        .setColor(client.config.color.main)
        .setAuthor(`Ticket Aperto`, `https://cdn.discordapp.com/emojis/833101350623117342.gif?size=512`)
        .setDescription(`Un Membro dello staff ti risponderà il prima possibile!`)
        .setFooter(`🔷 Estonija: ${interaction.guild.shardId}`, interaction.guild.iconURL())
        ...
        .addComponents([,
          ...
          .setEmoji(`🔷`)
          .setLabel(`Estonija`)
          .setURL(`https://discord.gg/estonija`)
        ])
          const row = new MessageActionRow()
          .addComponents([
            new MessageSelectMenu()
			      .setCustomId('tck_options')
			      .setPlaceholder('Gestisci il ticket')
			      .addOptions([
              { label: `Elimina Ticket`, description: `Chiusura Ticket`, value: `delete_ticket${s}`, emoji: `🗑️`},
              { label: `Fissa Ticket`, description: `only staff`, value: `pin_ticket${s}`, emoji: `📌`},
        ...
        .setTitle(`${client.emoji.wrong} È necessario il ruolo di amministratore per gestire questo ticket!`)
        .setDescription(`***Hai bisogno di <@&${role}> per chiudere questo ticket!***`)
        .setFooter(`Ticketing by ! Simo#0272 ${interaction.guild.shardId}`, interaction.guild.iconURL())
       ...
       const tcopener = interaction.guild.members.cache.get(opener.id);  interaction.channel.permissionOverwrites.edit(require('quick.db').fetch(`adminrole_${interaction.guild.id}${s}`), {
        ...
        const embed = new MessageEmbed()
        .setColor(client.config.color.success)
        .setTitle(`📌 Ticket Fissato!`)
        .setDescription(`> ***${interaction.user} ha fissato questo ticket!***`)
        .setFooter(` ${interaction.guild.shardId}`, interaction.guild.iconURL())
        db.set(`pinned_${interaction.channel.id}`, "fissato")
        interaction.channel.send({ embeds: [embed] })
        await interaction.deferUpdate({ ephemeral: false }).catch(() => {});
        ...
        .setColor(client.config.color.red)
        .setTitle(`${client.emoji.wrong} È necessario il ruolo di amministratore per gestire questo ticket!`)
        .setDescription(`***Non puoi chiudere questo ticket, chiedi ad uno staff!***`)
        .setFooter(`Ticketing by ! Simo#0272 ${interaction.guild.shardId}`, interaction.guild.iconURL())
                      if(!interaction.member.roles.cache.has(role)) {
                        return interaction.reply({ embeds: [norole], ephemeral: true})
                      }
...
.addComponents([
  ...
  .setLabel('Stai chiudendo il ticket')
