...
module.exports = {
    name: "manage_bot",
    description: "Gestisci il bot",
    type: 'CHAT_INPUT',
    premium: true,
    
    run: async (client, interaction, args) => {

      if(!client.config.developers.includes(interaction.user.id)) return interaction.followUp(`🔒 Devi essere un proprietario per usare questo comando!`)
    ...
    const control = new Discord.MessageEmbed()
      ...
      .setAuthor(`${client.user.username} | Pannello di controllo`, client.user.displayAvatarURL())
...
\`\`\`
<a:pingpong:906791053170987028> **Su di me:**
\`\`\`yml
${clientapp.description ? clientapp.description : "❌ NESSUNA DESCRIZIONE ANCORA!"}
\`\`\``
          : ""}
`)
